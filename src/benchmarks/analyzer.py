...
class BenchmarkAnalyzer:
    ...
    def get_scalability_analysis(self, architecture: str, ticket_type: str) -> Dict:
        ...
        server_latencies = [
            r['by_type'][ticket_type].get('avg_server_latency', 0)
            if r['by_type'].get(ticket_type) else 0
            for r in runs
        ]
        overhead_latencies = [
            r['by_type'][ticket_type].get('avg_overhead_latency', 0)
            if r['by_type'].get(ticket_type) else 0
            for r in runs
        ]
        ...
        return {
            ...
            "overhead_latencies": overhead_latencies,
    ...
    def generate_summary(self) -> str:
        ...
        for ticket_type in ["unnumbered", "numbered"]:
            ...
            for arch in ["rest", "rabbitmq"]:
                ...
                if any(analysis.get('server_latencies', [])):
                    summary.append(f"  Avg Server Latency (s): {analysis['server_latencies']}")
                    summary.append(f"  Avg Overhead Latency (s): {analysis['overhead_latencies']}")
